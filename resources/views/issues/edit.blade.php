...
@section('content')
    <div style="width:70%;display: block;margin-left: auto;margin-right: auto;">
        ...
        <form class="" action="/issues/{{ $issue->id }}" method="post" style="display:flex;justify-content:space-evenly;width:100%">
            ...
            <div class="" style="width:50%">
              ...
              <div class="form-group">
                  <label>Summary</label>
                  <input type="text" class="form-control" placeholder="Enter summary" name="summary" value="{{ old('summary') ?? $issue->summary }}" style="width:60%">
              ...
              <div class="form-group">
                  ...
                  <select class="form-control" name="status" style="width:25%">
                      @foreach($status as $key => $value)
                          @if(( old('status') ?? $issue->status ) == $key)
              ...
              <div class="form-group">
                  ...
                  <select class="form-control" name="priority" style="width:25%">
                      @foreach($priority as $key => $value)
                          @if(( old('priority') ?? $issue->priority ) == $key)
                              <option selected value="{{ $key }}">{{ $value }}</option>
                          @else
                              <option value="{{ $key }}">{{ $value }}</option>
                          @endif
            ...
            <div class="" style="width:50%">
              ...
              <div class="form-group">
                  ...
                  <select class="form-control" name="severity" style="width:25%">
                      @foreach($severity as $key => $value)
                          @if(( old('severity') ?? $issue->severity ) == $key)
                              <option selected value="{{ $key }}">{{ $value }}</option>
                          @else
                              <option value="{{ $key }}">{{ $value }}</option>
                          @endif
              ...
              <div class="form-group">
                  ...
                  <select class="form-control" name="reproducibility" style="width:25%">
                      @foreach($reproducibility as $key => $value)
                          @if(( old('reproducibility') ?? $issue->reproducibility ) == $key)
                              <option selected value="{{ $key }}">{{ $value }}</option>
                          @else
                              <option value="{{ $key }}">{{ $value }}</option>
                          @endif
              ...
              <div class="form-group">
                  ...
                  <br>
                  <textarea name="description" class="form-control" placeholder="Enter description" rows="8" cols="80" style="width:70%">{{ old('description') ?? $issue->description }}</textarea>
              </div><br>

              <div class="form-group">
                  <label>Steps</label>
                  <br>
                  <textarea name="steps" class="form-control" placeholder="Enter steps" rows="8" cols="80" style="width:70%">{{ old('steps') ?? $issue->steps }}</textarea>
              </div><br>
